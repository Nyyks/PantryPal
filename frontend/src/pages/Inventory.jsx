  ...
  const handleScan = useCallback(async (barcode) => {
    try {
      if (scanMode === 'add') {
        ...
        setScanResult({ type: 'success', message: `Added 1× ${result.product.name}`, product: result.product });
                      ...
                      <td>
                        {entryLocations.length > 0 ? (
                          <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                            {entryLocations.map(l => <span key={l} className="tag tag-accent" style={{ fontSize: 10 }}>{l}</span>)}
                          </div>
                      ...
                      <tr key={`entry-${entry.id}`} style={{ background: 'var(--bg-secondary)' }}>
                        ...
                        <td colSpan={2} style={{ fontSize: 12 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                            ...
                            {entry.best_before && (
                              <span className="tag tag-orange" style={{ fontSize: 10, marginLeft: 4 }}>BB: {entry.best_before}</span>
                            )}
                          </div>
                        </td>
