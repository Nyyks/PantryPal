...
def add_to_inventory():
    ...
    entry = InventoryEntry(
        ...
        location=location,
        scanned_barcode=normalize_barcode(barcode) if barcode else None,
    )
...
def shopping_scan():
    ...
    if add_to_inv:
        # Inherit location from last entry
        loc = None
        last = InventoryEntry.query.filter_by(product_id=product.id).filter(
            InventoryEntry.location.isnot(None), InventoryEntry.location != ''
        ).order_by(InventoryEntry.added_at.desc()).first()
        if last:
            loc = last.location
        entry = InventoryEntry(
            product_id=product.id, quantity=product.default_quantity or 1.0,
            scanned_barcode=normalize_barcode(barcode) if barcode else None,
            location=loc,
        )
        db.session.add(entry)
