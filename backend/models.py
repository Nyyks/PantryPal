...
class Product(db.Model):
    ...
    def all_barcodes(self):
        """Return all barcodes as strings."""
        codes = [b.barcode for b in self.barcodes]
        if self.barcode and self.barcode not in codes:
            codes.insert(0, self.barcode)
        return codes

    def all_barcodes_detailed(self):
        """Return all barcodes with labels."""
        seen = set()
        result = []
        for b in self.barcodes:
            if b.barcode not in seen:
                seen.add(b.barcode)
                result.append({'barcode': b.barcode, 'label': b.label, 'id': b.id})
        if self.barcode and self.barcode not in seen:
            result.insert(0, {'barcode': self.barcode, 'label': None, 'id': None})
        return result
    ...
    def to_dict(self):
        ...
        return {
            ...
            'barcodes_detailed': self.all_barcodes_detailed(),
            'name': self.name,
            'brand': self.brand,
...
class InventoryEntry(db.Model):
    ...
    scanned_barcode = db.Column(db.String(64), nullable=True)
    added_at = db.Column(db.DateTime, default=datetime.now)

    def to_dict(self):
        # Resolve variant label from product barcodes
        variant_label = None
        if self.scanned_barcode and self.product:
            for pb in self.product.barcodes:
                if pb.barcode == self.scanned_barcode:
                    variant_label = pb.label
                    break
        return {
            'id': self.id,
            'product_id': self.product_id,
